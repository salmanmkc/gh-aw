...
**Examples**: Valid paths include `/var/lib/mcp-gateway/payloads` (Unix) or `C:\gateway\payloads` (Windows). Invalid paths (MUST be rejected) include relative paths like `payloads` or `./payloads`, and empty/whitespace-only strings.
...
**Use Cases**: Mount read-only configuration files (`/etc/app/config.yaml:/app/config.yaml:ro`), provide access to large datasets (`/var/data/corpus:/data:ro`), enable result writing (`/var/output:/results:rw`), or share cache directories (`/tmp/cache:/app/cache:rw`).
...
A conforming implementation MUST pass tests covering: configuration parsing (19 tests including stdio/HTTP servers, variable expressions, custom types, volume mounts), protocol translation (8 tests for stdio/HTTP transports, tool signatures, connection handling), server isolation (8 tests for containers, environments, credentials, volume mounts), authentication (5 tests for tokens and exemptions), timeouts (5 tests for startup and tool invocations), health monitoring (9 tests for endpoints and versioning), configuration output (7 tests for stdout JSON generation), error handling (5 tests for startup/runtime failures), and gateway lifecycle (7 tests for shutdown and cleanup).

Test IDs follow the pattern T-{CATEGORY}-{NUMBER} where categories are: CFG (configuration), PTL (protocol translation), ISO (isolation), AUTH (authentication), TMO (timeout), HLT (health), OUT (output), ERR (error), LIFE (lifecycle).
...
This example demonstrates volume mounts, custom entrypoints, mixed transports (stdio + HTTP), registry field usage, tool filtering, and timeout configuration.

### Appendix B: Gateway Lifecycle Example

Calling `POST /close` with authentication initiates shutdown: stops accepting requests, waits for in-flight completion (max 30s), terminates containers (SIGTERM then SIGKILL after 10s), cleans up resources, returns HTTP 200 with `{"status": "closed", "serversTerminated": N}`, and exits. Subsequent calls return HTTP 410 Gone.

### Appendix C: Error Code Reference

Standard JSON-RPC errors: -32700 (parse), -32600 (invalid request), -32601 (method not found), -32602 (invalid params), -32603 (internal error). Gateway-specific: -32000 (server error), -32001 (unavailable), -32002 (timeout), -32003 (auth failed).

### Appendix D: Security Considerations

Implementations should protect credentials (no logging, isolated environments, memory clearing), secure network access (TLS, URL validation, CORS restrictions, rate limiting), harden containers (minimal privileges, resource limits, signed images, read-only filesystems, anomaly detection), and safeguard shutdown (authenticated `/close` endpoint, audit logging, DoS prevention via request timeouts, graceful SIGTERM before SIGKILL).
...
### Version 1.8.0 (Draft)
- Added `payloadDir` field for large payload file storage with absolute path validation

### Version 1.7.0 (Draft)
- Added volume mount support for stdio servers (host:container:mode format)

### Version 1.6.0 (Draft)
- Added custom server type extensibility via `customSchemas`

### Version 1.5.0 (Draft)
- Added `tools` field filtering for HTTP servers

### Version 1.4.0 (Draft)
- Relaxed authorization header format requirements
- Added HTTP connection failure handling requirements

### Version 1.3.0 (Draft)
- Added version fields to health endpoint (specVersion, gatewayVersion)

### Version 1.2.0 (Draft)
- Clarified stdout configuration output requirements (headers, authorization)

### Version 1.1.0 (Draft)
- Added `/close` endpoint for graceful shutdown
