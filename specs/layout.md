...
This document catalogs all file paths, folder names, artifact names, and other patterns used across our compiled GitHub Actions workflows (`.lock.yml` files). The specification is based on analysis of **142 lock files** in the `.github/workflows/` directory.
...
| `actions/checkout` | `8e8c483db84b4bee98b60c0593521ed34d9990e8` | Checks out repository code | Used in almost all workflows for accessing repo content |
...
Common workflow trigger types used across workflows:

| Trigger | Description | Usage Pattern |
|---------|-------------|---------------|
| `workflow_dispatch` | Manual workflow dispatch | Used for on-demand workflow execution |
| `schedule` | Scheduled (cron-based) execution | Used for daily/periodic workflows |
| `issue_comment` | Issue comment events | Triggered when comments are added to issues |
| `issues` | Issue lifecycle events | Triggered on issue open, close, edit, etc. |
| `pull_request` | Pull request events | Triggered on PR lifecycle events |
| `pull_request_review_comment` | PR review comment events | Triggered on review comments |
| `push` | Push to repository | Triggered on code pushes |
| `discussion` | Discussion events | Triggered on discussion lifecycle events |
| `discussion_comment` | Discussion comment events | Triggered on discussion comments |
| `workflow_run` | Workflow completion trigger | Triggered when another workflow completes |

## Go Constants Reference

### Key Constants from `pkg/workflow/`

```go
// Script and action paths
...
*Auto-generated by Layout Specification Maintainer workflow - 2026-01-29*
